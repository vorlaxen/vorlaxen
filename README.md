# Vorlaxen

Software developer from Türkiye working on backend systems, DevOps, open-source software and modern web applications.

I build APIs, developer tools, deployment workflows and full-stack products—primarily with TypeScript and Node.js. I also work with embedded systems and hardware when a project goes beyond the browser.

Currently building projects under **Vorlaxen Labs** and developing software at **Xristal Ltd. Şti.**.

## Toolbox

* **Backend:** TypeScript, Node.js, Express, NestJS, Sequelize
* **Frontend:** React, Vite, Tailwind CSS
* **Data:** PostgreSQL, MySQL, Redis
* **DevOps:** Docker, Coolify, CapRover, Git
* **Also working with:** Python, C# and embedded systems

## How I Work

I care about clear architecture, maintainable code, reliable deployments, efficient database design and security built into the system from the beginning.

## Links

[Website](https://www.vorlaxen.com) · [Vorlaxen Labs](https://docs.vorlaxen.com) · [LinkedIn](https://www.linkedin.com/in/vorlaxen)
